...
describe("shouldUseCompactComposerPrimaryActions", () => {
  ...
  it("stays expanded when the action set is not wide", () => {
    expect(shouldUseCompactComposerPrimaryActions(100, { hasWideActions: false })).toBe(false);
    expect(shouldUseCompactComposerPrimaryActions(100)).toBe(false);
  });
});
