...
function parseWindowsProcessRows(output: string): ReadonlyArray<ProcessRow> {
  ...
  try {
    ...
    const records = globalThis.Array.isArray(parsed) ? parsed : [parsed];
...
function makeResult(input: {
  ...
  const totalRssBytes = processes.reduce((total, childProcess) => total + childProcess.rssBytes, 0);
  const totalCpuPercent = processes.reduce(
    (total, childProcess) => total + childProcess.cpuPercent,
    0,
  );
...
const runProcess = Effect.fn("ProcessDiagnostics.runProcess")(
  function* (input: {
    readonly command: string;
          ...
          Effect.try({
            try: () => {
              ...
              return {
                ...
                signaled: true,
                message: null,
              } satisfies ServerSignalProcessResult;
            },
            catch: (cause) =>
