...
function makeEmptyDiagnostics(input: {
  ...
  return {
    ...
    partialFailure: input.partialFailure ? true : null,
...
export function aggregateTraceDiagnostics(
  ...
  for (const file of input.files) {
    for (const line of file.text.split(/\r?\n/)) {
      ...
      recordCount += 1;
      firstSpanAt =
        startedAt && (firstSpanAt === null || epochMillis(startedAt) < epochMillis(firstSpanAt))
          ? startedAt
          : firstSpanAt;
      lastSpanAt =
        lastSpanAt === null || epochMillis(endedAt) > epochMillis(lastSpanAt)
          ? endedAt
          : lastSpanAt;
      ...
      if (isFailure) {
        ...
        const isLatestFailure =
          !existing || epochMillis(endedAt) > epochMillis(existing.lastSeenAt);
      ...
      if (Array.isArray(parsed.events)) {
        for (const rawEvent of parsed.events) {
          ...
          latestWarningAndErrorLogs.push({
            spanName: name,
            level,
            message: toStringValue(rawEvent.name)?.trim() ?? "Log event",
            seenAt: unixNanoToIso(rawEvent.timeUnixNano) ?? endedAt,
            traceId,
            spanId,
          });
        }
  ...
  return {
    ...
    commonFailures: [...failuresByKey.values()]
      .toSorted(
        (left, right) =>
          right.count - left.count || epochMillis(right.lastSeenAt) - epochMillis(left.lastSeenAt),
      )
      .slice(0, TOP_LIMIT),
    latestFailures: latestFailures
      .toSorted((left, right) => epochMillis(right.endedAt) - epochMillis(left.endedAt))
      .slice(0, RECENT_LIMIT),
    latestWarningAndErrorLogs: latestWarningAndErrorLogs
      .toSorted((left, right) => epochMillis(right.seenAt) - epochMillis(left.seenAt))
      .slice(0, RECENT_LIMIT),
    partialFailure: input.partialFailure ? true : null,
    error: input.error ?? null,
  };
...
export const make = Effect.fn("makeTraceDiagnostics")(function* () {
  ...
  const read: TraceDiagnosticsShape["read"] = Effect.fn("TraceDiagnostics.read")(
    function* (options) {
      const readAt = options.readAt ?? (yield* nowIso);
