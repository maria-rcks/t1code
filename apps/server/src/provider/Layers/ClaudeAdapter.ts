    ...
    Effect.gen(function* () {
      ...
      const handleAskUserQuestion = (
        ...
        Effect.gen(function* () {
          ...
          // `id` must equal the full question text: recent Claude SDKs look
          // up answers by question text when rendering the tool result.
...
export function makeClaudeAdapterLive(options?: ClaudeAdapterLiveOptions) {
  return Layer.effect(ClaudeAdapter, makeClaudeAdapter(decodeClaudeSettings({}), options));
}
