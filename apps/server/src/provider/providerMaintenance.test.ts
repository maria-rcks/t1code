...
const driver = (value: string) => decodeProviderDriverKind(value);
...
describe("providerMaintenance", () => {
  ...
  it("switches package-managed providers to vite-plus updates", () => {
    const tempDir = makeTempDir("t3-vite-plus-capabilities");
  ...
  it("switches package-managed providers to bun updates", () => {
    const tempDir = makeTempDir("t3-bun-capabilities");
  ...
  it("switches package-managed providers to pnpm updates", () => {
    const tempDir = makeTempDir("t3-pnpm-capabilities");
  ...
  it("switches native-package-tool to native updates when installed natively", () => {
    const tempDir = makeTempDir("t3-native-package-tool-native-capabilities");
  ...
  it("switches scoped-package-tool to native upgrades when installed standalone", () => {
    const tempDir = makeTempDir("t3-scoped-package-tool-native-capabilities");
  ...
  it("switches native-package-tool to Homebrew updates when installed with Homebrew", () => {
    expect(
      nativePackageToolUpdate.resolve({
  ...
  it("switches scoped-package-tool to Homebrew updates when installed with Homebrew", () => {
    expect(
      scopedPackageToolUpdate.resolve({
  ...
  it.effect("keeps npm updates for binaries symlinked into npm's global tree", () =>
    Effect.gen(function* () {
      const tempDir = makeTempDir("t3-npm-capabilities");
  ...
  it.effect("uses realpath resolution when detecting pnpm global symlinks", () =>
    Effect.gen(function* () {
      const tempDir = makeTempDir("t3-pnpm-realpath-capabilities");
      const binDir = path.join(tempDir, "bin");
      const packageBinDir = path.join(
        tempDir,
        ".local",
