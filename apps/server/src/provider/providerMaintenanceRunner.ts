...
export const make = Effect.fn("ProviderMaintenanceRunner.make")(function* () {
  ...
  const applyUpdateState = Effect.fn("ProviderMaintenanceRunner.applyUpdateState")(function* (
    provider: ServerProvider,
  ) {
    const state = (yield* Ref.get(maintenanceActionStatesRef)).get(provider.instanceId);
  ...
  const listProviders = Effect.fn("ProviderMaintenanceRunner.listProviders")(function* () {
    ...
    return yield* Effect.forEach([...snapshots, ...unavailable], applyUpdateState, {
      concurrency: "unbounded",
    });
  });
  ...
  const verifyRefreshedProvider = (
    ...
    Effect.gen(function* () {
      ...
      const instance = instances.find(
        (candidate) => candidate.driverKind === provider && candidate.instanceId === instanceId,
      );
      if (!instance) {
        return {
          providers: yield* listProviders(),
          verifiedProviders: [],
        } satisfies VerifiedProviderRefresh;
      }

      const refreshedProvider = yield* instance.snapshot.refresh;
      const verifiedProviders = yield* enrichProviderSnapshotWithVersionAdvisory(
        refreshedProvider,
        maintenanceCapabilities,
      ).pipe(
        Effect.provideService(HttpClient.HttpClient, httpClient),
        Effect.map((verifiedProvider) => [verifiedProvider] as const),
        Effect.catchCause((cause) =>
          Effect.logWarning("Provider post-update version verification failed", {
            provider,
            cause: Cause.pretty(cause),
          }).pipe(Effect.as([refreshedProvider] as const)),
        ),
      );
    ...
    const runProviderUpdate = Effect.fn("ProviderMaintenanceRunner.runProviderUpdate")(
      function* () {
        const finish = (state: ServerProviderUpdateState) =>
          setUpdateState(instanceId, state).pipe(Effect.map((providers) => ({ providers })));
