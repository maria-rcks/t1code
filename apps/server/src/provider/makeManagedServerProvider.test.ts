...
describe("makeManagedServerProvider", () => {
  it.effect("runs the initial provider check in the background", () =>
    Effect.scoped(
      Effect.gen(function* () {
        const checkCalls = yield* Ref.make(0);
        const releaseCheck = yield* Deferred.make<void>();
        const provider = yield* makeManagedServerProvider<TestSettings, never>({
  ...
  it.effect("streams supplemental snapshot updates after provider checks", () =>
    Effect.scoped(
      Effect.gen(function* () {
  ...
  it.effect("ignores stale enrichment callbacks after a newer refresh advances generation", () =>
    Effect.scoped(
      Effect.gen(function* () {
        ...
        const firstPublish = publishCallbacks[0];
        const secondPublish = publishCallbacks[1];
        if (!firstPublish || !secondPublish) {
          throw new Error("Expected enrichment callbacks to be registered.");
        }

        yield* firstPublish(enrichedSnapshot);
        yield* secondPublish(enrichedSnapshotSecond);
