...
describe("ProviderCommandReactor", () => {
  ...
  it("generates a title for the first turn when the thread still has the default title", async () => {
    const harness = await createHarness();
    harness.generateThreadTitle.mockImplementation(() =>
      Effect.succeed({
        title: "Investigate TUI settings crash",
      }),
    );
    const now = new Date().toISOString();
    ...
    await Effect.runPromise(
      harness.engine.dispatch({
        ...
        commandId: CommandId.make("cmd-thread-title-default"),
        threadId: ThreadId.make("thread-1"),
        title: "New thread",
      }),
    );
    ...
    await Effect.runPromise(
      harness.engine.dispatch({
        ...
        message: {
          ...
          role: "user",
          text: "settings keeps failing when I open it",
          attachments: [],
        },
        interactionMode: DEFAULT_PROVIDER_INTERACTION_MODE,
        runtimeMode: "approval-required",
    ...
    await waitFor(async () => {
      ...
      return (
        ...
        "Investigate TUI settings crash"
      );
    });
  });
  ...
  it("uses model selection instance info to choose the provider driver", async () => {
    ...
    await Effect.runPromise(
      harness.engine.dispatch({
        type: "thread.turn.start",
        commandId: CommandId.make("cmd-turn-start-cursor-instance"),
        threadId: ThreadId.make("thread-1"),
        message: {
  ...
  it("rejects a turn when the requested model belongs to a different provider", async () => {
    ...
    await Effect.runPromise(
      harness.engine.dispatch({
        type: "thread.turn.start",
        commandId: CommandId.make("cmd-turn-start-model-provider-mismatch"),
        threadId: ThreadId.make("thread-1"),
        message: {
