...
export function normalizeThreadTerminalState(state: ThreadTerminalState): ThreadTerminalState {
  ...
  const normalized: ThreadTerminalState = {
    ...
    terminalFullScreen: state.terminalOpen ? state.terminalFullScreen === true : false,
    terminalHeight:
...
export function setThreadTerminalOpen(
  ...
  return {
    ...normalized,
    terminalOpen: open,
    terminalFullScreen: open ? normalized.terminalFullScreen : false,
  };
}

export function setThreadTerminalFullScreen(
  state: ThreadTerminalState,
  fullScreen: boolean,
): ThreadTerminalState {
  const normalized = normalizeThreadTerminalState(state);
  const nextFullScreen = normalized.terminalOpen ? fullScreen : false;
  if (normalized.terminalFullScreen === nextFullScreen) {
    return normalized;
  }
  return {
    ...normalized,
    terminalFullScreen: nextFullScreen,
  };
}
