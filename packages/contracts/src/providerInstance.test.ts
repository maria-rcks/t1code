  ...
  for (const { schemaName, decode } of cases) {
    describe(schemaName, () => {
      ...
      it("rejects ids longer than 64 characters", () => {
        expect(() => decode("a".repeat(65))).toThrow();
...
describe("ProviderInstanceRef", () => {
  ...
  it("decodes a fork-defined driver ref", () => {
    const ref = decodeProviderInstanceRef({
...
describe("defaultInstanceIdForDriver", () => {
  it("preserves the legacy driver-as-instance-id mapping", () => {
    expect(defaultInstanceIdForDriver(decodeProviderDriverKind("claudeAgent"))).toBe("claudeAgent");
  });
});
...
describe("ProviderInstanceConfigMap", () => {
  it("decodes a multi-instance map mixing first-party and fork drivers", () => {
    ...
    expect(decoded["codex_personal" as ProviderInstanceId]?.driver).toBe("codex");
    expect(decoded["codex_work" as ProviderInstanceId]?.config).toEqual({
      homePath: "~/.codex_work",
    });
    expect(decoded["ollama_local" as ProviderInstanceId]?.driver).toBe("ollama");
  });
